...
<form action="<?php echo GTPAY_TRANS_REQUEST ?>" method="post">
    ...
    <input type='hidden' name='gtpay_tranx_hash' value='<?php echo hash("sha512", $_GET['card-order']->getValue('id') . bcmul($_GET['plan']->getValue('amount'), 100) . CONTEXT_PATH . '/hotspot/notify2' . GTPAY_HASH_KEY); ?>' />
    <input type='hidden' name='gtpay_tranx_id' value='<?php echo $_GET['card-order']->getValue('id') ?>' />
