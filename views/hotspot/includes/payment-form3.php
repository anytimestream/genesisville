<div class="trans-status">
    <h3>Make Payment</h3>
    <div class="content">
        <p>Transaction Reference: <?php echo $_GET['card-order']->getValue('transaction_id') ?></p>
...
<form action="<?php echo GTPAY_TRANS_REQUEST ?>" method="post">
    ...
    <input type='hidden' name='gtpay_tranx_hash' value='<?php echo hash("sha512", $_GET['card-order']->getValue('transaction_id') . bcmul($_GET['plan']->getValue('amount'), 100) . CONTEXT_PATH . '/hotspot/notify2' . GTPAY_HASH_KEY); ?>' />
    <input type='hidden' name='gtpay_tranx_id' value='<?php echo $_GET['card-order']->getValue('transaction_id') ?>' />
    <input type='hidden' name='gtpay_cust_id' value='<?php echo $_GET['card-order']->getValue('phone') ?>' />
    ...
    <input type='hidden' name='gtpay_tranx_amt' value='<?php echo bcmul($_GET['plan']->getValue('amount'), 100) ?>' />
    <div class=row>
